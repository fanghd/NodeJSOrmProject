...
ServerController.getServiceCount = async (ctx) => {
	try {
		let rsp = await ServerService.getServiceCount();
		ctx.makeResObj(200, '', rsp[0]);
	} catch (e) {
		logger.error(e, ctx);
...
ServerController.getDowngradeList = async (ctx) => {
	let {application, server_name} = ctx.paramsObj;
	try {
		if (!await AuthService.hasDevAuth(application, server_name, ctx.uid)) {
			ctx.makeNotAuthResObj();
		} else {
			let ret = await ServerService.getServerDowngradeList({application, server_name});
			ctx.makeResObj(200, '', ret);
		}
	} catch (e) {
		logger.error('[getDowngradeList]', e, ctx);
		ctx.makeErrResObj();
	}
}
